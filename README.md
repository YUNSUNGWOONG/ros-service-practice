...
> **왜 이 시나리오인가**: 단순 Topic 통신(초급)을 넘어 커스텀 srv 빌드 + 비동기 +
> 예외 처리를 한 번에 다루는 중급 주제라 서비스 통신의 전체 파이프라인을 체감할 수
> 있습니다. "좌표 승인"이라는 조건 검증 로직이 직관적이라 통신 구조 자체에 집중할 수
> 있고, 별도 물리 시뮬레이터(Isaac Sim 등) 없이 순수 노드 간 통신만으로 검증됩니다.
